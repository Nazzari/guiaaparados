<?
/**
 * Classe de administra��o e intera��o com banco de dados a partir de uma tabela base
 * 
 * <i>Criada em: 23/01/2009
 * �ltima Altera��o: 17/02/2010</i>
 * 
 * @author Eduardo Firmino Leit�o
 * @version 1.7.9
 * @copyright Copyright � 2009, Eduardo Firmino Leit�o = www.eduardofirmino.com
 * @package Class
 */
class cAutomates {

	/**
 	 * Vari�vel de nome da tabela base
 	*/
	private $v_tabelaBase;
	
	/**
 	 * Vari�vel do objeto de conex�o com o banco de dados
 	*/
	private $cCon;
	
	/**
 	 * Vari�vel para guardar a pagina��o caso queira mostrar
 	*/
	public $v_paginacao;
	
	/**
 	 * Vari�vel para guardar o caminho dos arquivos de upload
 	*/
	public $v_caminho_imagem = "../../fotos/";
	
	/**
	 * M�todo Construtor que seta a tabela base
	...
	*/
	...
	/**
	 * M�todo para retornar informa��es do coment�rio da tabela base
	 * @return Array $result � Informa��es da tabela
	*/
	...
	/**
	 * M�todo para retornar um ou mais campos da tabela base
	...
	*/
	...
	/**
	 * M�todo para retornar dados da tabela base
	 * @param int $codigo � C�digo do cadastro que queira buscar os dados (opcional)
	 * @param String $where � Op��es de where no sql de consulta
	 * @param String $order � Op��es de order no sql de consulta
	 * @param String $order � Op��es de limit no sql de consulta
	 * @return Array $result � Dados consultados
	*/
	...
	/**
	 * M�todo para atualizar dados como cadastrar e modificar
	 * @param Array $dados � Dados para atualizar no banco
	 * @param int $codigo � C�digo do cadastro caso queira modificar (opcional)
	 * @param String $where � Op��es de where no sql
	*/
	...
	/**
	 * M�todo para deletar dados da tabela base
	 * @param int $codigo � C�digo do cadastro que queira deletar (opcional)
	 * @param String $where � Op��es de where no sql
	*/
	function mDeletaDados($codigo="",$where="") {
		if (($codigo) || ($where)) {
			...
			exit("Erro: C�digo ou condi��o em branco");
		}
	}
	
	/**
	 * M�todo para converter datas
	...
	*/
	...
	/**
	 * M�todo para gerar pagina��o de resultados
	 * @param int $pagina � N�mero da p�gina atual
	 * @param int $resultadosPagina � N�mero de resultados por p�gina
	 * @param int $totalResultados � Total de resultados da consulta
	 * @param string $link � Link para a pagina��o
	 * @param int $nrPaginas � N�mero de p�ginas pra fazer a pagina��o
	...
	*/
	public function mPaginacao($pagina=0,$resultadosPagina=10,$totalResultados=0,$link="",$nrPaginas=5){
		if ($totalResultados > $resultadosPagina) $s_msg   = "P�ginas:";
		...
		$s_msg .= ($pagina+1) <= $s_resultados ? " <a href=\"".$link.($pagina+1)."\">Pr�xima</a>" : "";
	...
	/**
	 * Vari�veis extras para administra��o
	*/
	...
	public $v_campo_padrao;			//define um campo com valor padr�o (array)
	public $v_campo_exemplo;		//define um exemplo para determinado campo (array)
	public $v_html_extra_p;			//define um html extra ao p em determinado campo que s�o os itens de cadastro do sistema (array)
	public $v_html_extra_campo;		//define um html extra a um determinado campo que s�o os itens de cadastro do sistema (array)
	public $v_html_extra_admin;		//adiciona um html extra ao lado dos bot�es no link de admin (principal)
	public $v_html_extra_insert;	//adiciona um html extra ao lado dos bot�es no link de cadastro
	public $v_html_extra_manage;	//adiciona um html extra ao lado dos bot�es no link de manage
	public $v_html_extra_update;	//adiciona um html extra ao lado dos bot�es no link de edi��o
	public $v_redireciona_cadastro;	//redireciona o cadastro para outro link (array) campo e link
	public $v_redireciona_alteracao;//redireciona a altera��o para outro link seguido das vari�veis atuais
	public $v_tabela_backup;		//seta uma tabela de backup para guardar dados em seguran�a
	
	/**
	 * M�todo para iniciar a administra��o da tabela base
	*/
	function mAdmin() {
