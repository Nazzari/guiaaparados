...
switch($cUrl->mGetParam(1)) {
	...
	case "depoimentos":
		if ($cUrl->mGetParam(2) == "enviar") {
			if ($_POST['nome'] != "") {
                ...
                $headers .= "Content-type: text/html; charset=iso-8859-1\n";
                $headers .= "From: sistema@guiaaparadosdaserra.com.br\n";
                $assunto = "Guia Aparados da Serra - Adventure Aparados (Depoimento)";
			...
			$cTpl = new cTemplate("tpl.depoimentos.htm");
	...
	***************************/
	case "roteiro-familia":
		...
		$cTpl = new cTemplate("tpl.internas.htm");
		...
		$cTpl->mSet("tit_int", "ROTEIRO FAMÍLIA APARADOS DA SERRA");
		$cTpl->mShow("inicio");
	...
	***************************/
	case "passagens":
		...
		$cTpl = new cTemplate("tpl.passagens.htm");
	...
	***************************/
	case "4x4":
		...
		$cTpl->mSet("tit_int", "4x4 NOS APARADOS DA SERRA");
	...
	***************************/
	case "rapel":
		...
		$cTpl->mSet("tit_int", "RAPEL NOS APARADOS DA SERRA");
	...
	***************************/
	case "canionismo":
		...
		$cTpl->mSet("tit_int", "CANIONISMO APARADOS DA SERRA");
		$cTpl->mShow("inicio");
	...
	**************************/
	case "aventuras-personalizadas":
		...
		if($cUrl->mGetParam(2) == "enviar"){
			...
			$cTpl = new cTemplate("tpl.monte-seu-pacote.htm");
	...
	**************************/
	case "pacotes":
		...
		if ($cUrl->mGetParam(2)) {
			...
			$cTpl = new cTemplate("tpl.pacotes-interna.htm");
			...
			if (count($fotos) > 0) {
				...
				for ($i=0; $i < count($fotos); $i++) { 
					$cTpl->mSet("tt_foto",$fotos[$i]['tt_foto']);
					$cTpl->mSet("foto",$cGd->mGeraGd($caminhoFotos,$fotos[$i]['nm_foto'],360,2));
					$cTpl->mSet("link",$cGd->mGeraGd($caminhoFotos,$fotos[$i]['nm_foto'],500,2));
					$cTpl->mShow("while");				
				}
				$cTpl->mShow("fim_while");
	...
	**************************/
	case "passeios":
		...
		if ($cUrl->mGetParam(2)) {
			...
			$cTpl = new cTemplate("tpl.passeios-interna.htm");
	...
	**************************/
	case "informacoes":
		...
		$cTpl = new cTemplate("tpl.politica.htm");
	...
	**************************/
	case "guias":
		...
		if ($cUrl->mGetParam(2)) {
			...
			$cTpl = new cTemplate("tpl.guias-int.htm");
	...
	**************************/
	case "mountain-bike":
		...
		$cTpl->mSet("tit_int", "MOUNTATIN BIKE APARADOS DA SERRA");
		$cTpl->mShow("inicio");
	...
	**************************/
	default:
		...
		// 	if (count($foto) > 0) {
		// 		$cTpl->mSet("foto",$cGd->mGeraGd($caminhoFotos,$foto[0]['nm_foto'],360,2));
		// 	} else {
