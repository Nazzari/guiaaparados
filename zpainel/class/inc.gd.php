...
class cGd extends cErros {
	...
	/**
	 * Qualidade da imagem que você pretende ver
	 */
	private $v_quality;
	
	/**
	 * Tipo de imagem que você pretende ver (1 = Normal e 2 = Em pé)
	 */
	/*private $tipo;*/
	...
	/**
	 * Largura padrão para se basear nos calculos da foto
	 */
	private $v_width_base;
	
	/**
	 * Altura padrãopara se basear nos calculos da foto
	 */
	private $v_height_base;
	
	/**
	 * Cópia fiel da imagem
	 */
	...
	*/
	 ...
	 * @param int $p_width » Largura máxima da imagem
	 * @param int $p_miniatura » Tipo da miniatura 1 = min_, 2 = med_ e 3 = max_
	 * @param int $p_tp » Posição da imagem 1 = De lado e 0 = Em pé
	 * @param int $p_quality » Qualidade da imagem 1 = Boa e 0 = Ruim
	 * @param int $p_borda » Borda na imagem 1 = Sim e 0 = Não (padrão)
	 * @param String $p_fundo » Cor de fundo da imagem "255,255,255" (padrão)
	  * @param String $p_cor_borda » Cor da borda da imagem "0,0,0" (padrão)
	 */
	function mGeraGd($p_caminho, $p_image, $p_width, $p_miniatura=1, $p_tp=1, $p_quality=1, $p_borda=0, $p_fundo="255,255,255", $p_cor_borda="0,0,0") {
		...
		// Caso largura tiver vazio seta padrão 100
		$this->v_width_max = $p_width == "" ? 100 : $p_width;

		// Caso miniatura tiver vazio seta padrão 1
		...
		// Caso qualidade tiver vazio seta padrão 1 = Boa
		...
		if ($this->mVerificaExiste()) {
			...
			// Verifica se é max_ para excluir a original
	...
	*/
	...
	function mValidaImagem(){
		if ($this->v_ext != "jpg" && $this->v_ext != "jpeg" && $this->v_ext != "gif") {
			exit("Arquivo não é uma imagem valida, é válido apenas » jpg ou gif!");	
		}
	}
	...
	/**
	 * Verifica se a imagem original já não está no tamanho desejado
	*/
	...
	function mRedimensionaImageCopia(){
		...
		// Redimenciona para padrão do width
		...
		// Redimenciona para padrão do height
		while($this->v_height_img_resize > $this->v_height_max){
			$this->v_width_img_resize = $this->v_width_img_resize/1.001;
			$this->v_height_img_resize = $this->v_height_img_resize/1.001;
		}
		
		// Adiciona 3px para não exibir espaco em baixo
