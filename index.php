...
switch ($cUrl->mGetParam(1)) {
    ...
    **********************************/
    case "videos-int":
        ...
        $cTpl = new cTemplate("tpl.videos-int.htm");
    ...
    /*********************************
        AGENDA DE PASSEIOS
    **********************************/
    case "agenda-de-passeios":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.agenda-de-passeios.htm");
    ...
    **********************************/
    case "telefones-uteis":
        ...
        $cTpl = new cTemplate("tpl.telefones-uteis.htm");
    ...
    **********************************/
    case "mountain-bike":
        ...
        $cTpl = new cTemplate("tpl.mountain-bike.htm");
    ...
    **********************************/
    case "os-trekkings":
        ...
        $cTpl = new cTemplate("tpl.os-trekkings.htm");
    ...
    **********************************/
    case "oque-fazer":
        ...
        $cTpl = new cTemplate("tpl.oque-fazer.htm");
    ...
    /*********************************
        TREKKING TRAVESSIAS
    **********************************/
    case "trekking-travessias":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.trekking-travessias.htm");
    ...
    /*********************************
        MAPAS COMO CHEGAR
    **********************************/
    case "mapas-como-chegar":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.mapas-como-chegar.htm");
    ...
    /*********************************
        COMPRAS E SERVI�OS
    **********************************/
    case "compras-servicos":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.compras-servicos.htm");
    ...
    /*********************************
        AG�NCIAS GUIAS
    **********************************/
    case "agencias-guiasXXX":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.agencias-guias.htm");
    ...
    **********************************/
    case "ondecomer":
        ...
        $cTpl = new cTemplate("tpl.onde-comer.htm");
    ...
    /*********************************
        ONDE FICAR - A SER FEITO
    **********************************/
    case "ondeficar":

        include "principalInicio.php";

        $cTpl = new cTemplate("tpl.onde-ficar.htm");
    ...
    case "formBusca":
        ...
        /* Agencias / Guias */
        if (count($agencias) > 0) {
            $cTpl->mSet("nmModulo", "Ag�ncias / Guias");
        ...
        /* Compras e Servi�os */
        if (count($compras) > 0) {
            $cTpl->mSet("nmModulo", "Compras e Servi�os");
        ...
        if (count($dicas) > 0) {
            ...
            $cTpl->mSet("nmModulo", "Telefones �teis");
        ...
        if (count($excursao) > 0) {
            ...
            $cTpl->mSet("nmModulo", "Excurs�o para grupos / col�gios / universidade");
        ...
        if (count($expedicao) > 0) {
            ...
            $cTpl->mSet("nmModulo", "Expedi��o aparados 4x4");
        ...
        if (count($atividades) > 0) {
            ...
            $cTpl->mSet("nmModulo", "N�vel de atividades");
        ...
        if (count($aparados) > 0) {
            ...
            $cTpl->mSet("nmModulo", "O que � Aparados da Serra");
    ...
    case "noticias":
        //include "principalInicio.php";
        principalInicio("Not�cias - Guia Aparados da Serra");

        if ($cUrl->mGetParam(2)) {

            $cTpl = new cTemplate("tpl.internas-txt-ft.htm");

            $cAutomates = new cAutomates("noticias");
            $noticia = $cAutomates->mRetornaDados($cUrl->mGetParam(2));

            $cTpl->mSet("titulo", utf8_encode($noticia[0]['tt_noticia']));
            $cTpl->mSet("descricao", utf8_encode(nl2br($noticia[0]['ds_noticia'])));
            ...
            for ($i = 0; $i < count($fotos); $i++) {
                ...
                $cTpl->mSet("foto", "/../../".$cGd->mGeraGd($caminhoFotos, $fotos[$i]["nm_foto"], 350, 1));
            ...
            for ($i = 0; $i < count($noticia); $i++) {
                ...
                if (count($fotos) > 0) {
                    ...
                    $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $fotos[0]["nm_foto"], 350, 1));
    ...
    case "informe-se":
        ...
        if ($cUrl->mGetParam(2)) {
            ...
            $cTpl->mSet("titulo", utf8_encode($informe[0]['tt_informe']));
            $cTpl->mSet("descricao", utf8_encode(nl2br($informe[0]['ds_informe'])));
    ...
    case "translados-transfer":
        ...
        $cTpl->mSet("titulo", "Translados / Transfer");
    ...
    case "pacotes-e-passeios-personalizados":
        ...
        $cTpl->mSet("titulo", "Tour para Itaimb�zinho");
        $cTpl->mSet("descricao", nl2br($personalizado[0]['ds_personalizado']));
        $cTpl->mShow("inicio");
    ...
    case "galeria-de-fotos":
        ...
        $cTpl->mSet("ds_galeria", utf8_encode(nl2br($galeria[0]['ds_galeria'])));
        $cTpl->mShow("inicio");
    ...
    case "compre-o-guia-impresso":
        ...
        $cTpl->mSet("ds_compre", utf8_encode(nl2br($compre[0]['ds_compre'])));
    ...
    case "anuncie":
        if ($cUrl->mGetParam(2) == "enviar") {
            if ($_POST['nome'] != "") {
                ...
                $conteudo.= "<p><strong>Observa��es</strong><br />" . $_POST["mensagem"] . "</p>";
                ...
                $assunto = "Guia Aparados da Serra (Anuncie)";

                if (!mail($anuncie[0]["email_anuncie_dados"],$assunto,$body,$headers, "-r"."sistema@guiaaparadosdaserra.com.br")){

                    $headers .= "Return-Path: sistema@guiaaparadosdaserra.com.br\n";

                    if (!mail($anuncie[0]["email_anuncie_dados"],$assunto,$body,$headers) ){
                        echo "Erro ao enviar emails.";
                    }
                    else {
    ...
    case "fale-conosco":
        if ($cUrl->mGetParam(2) == "enviar") {
            ...
            $cTpl->mSet("rodape", utf8_encode(nl2br($contato[0]['ds_faleconosco_dados'])));
    ...
    case "compras-e-servicos":
        ...
        if ($cUrl->mGetParam(2) == "enviar") {
            ...
            principalInicio("Compras e Servi�os > ". $compra[0]['nm_cidade'] ." - Guia Aparados da Serra");
            $cTpl->mSet("nm_cidade", $compra[0]['nm_cidade']);

            $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $compra[0]["ft_compra"], 350, 1));
            ...
            if ($compra[0]['em_compra'] != "" && isset($compra[0]['em_compra'])) {
                ...
                $cTpl = new cTemplate("tpl.email.htm");
                $cTpl->mSet("site", "Guia Aparados da Serra");
                $cTpl->mSet("pagina", "Notifica��o");
                $cTpl->mSet("hora", date("H:i"));
                $cTpl->mSet("dia", date("d/m/Y"));

                $conteudo.= "<p>Mais um turista acaba de solicitar seu contado atrav�s do portal www.guiaaparadosdaserra.com.br. Boas vendas. <br/> 
                                Mensagem autom�tica. Portais da Ag�ncia Guia Aparados. <br/>
                ...
                $PHPMailer->Subject = "Sistema Autom�tico Notifica��o";
                $PHPMailer->Body = $body;
                $PHPMailer->AltBody = $body;
                $PHPMailer->WordWrap = 50;
            ...
            if($cUrl->mGetParam(2) == 3){
                ...
                principalInicio("Compras e Servi�os > Outros - Guia Aparados da Serra");
            ...
            for ($i = 0; $i < count($compra); $i++) {
                ...
                $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $compra[$i]["ft_compra"], 350, 1));
    ...
    case "agencias-guias":
        ...
        if ($cUrl->mGetParam(2) == "enviar") {
            ...
            $cTpl = new cTemplate("tpl.agencia-guias-enviar.htm");

            $cAutomates = new cAutomates("agencias");

            $agencia = $cAutomates->mRetornaDados($cUrl->mGetParam(3));
            utf8_encode(principalInicio("Agências / Guias > ". $agencia[0]['nm_cidade'] ." - Guia Aparados da Serra"));
            $cTpl->mSet("nm_cidade", utf8_encode($agencia[0]['nm_cidade']));

            $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $agencia[0]["ft_agencia"], 350, 1));
            $cTpl->mSet("link", $cGd->mGeraGd($caminhoFotos, $agencia[0]["ft_agencia"], 500, 3));
            $cTpl->mSet("cd_agencia", $agencia[0]['cd_agencia']);
            $cTpl->mSet("nm_pousada", utf8_encode($agencia[0]['nm_agencia']));
            $cTpl->mSet("end_pousada", utf8_encode($agencia[0]['end_agencia']) . " - " . utf8_encode($agencia[0]['nm_cidade']));
            ...
            $cTpl->mSet("st_pousada", $link);
            $cTpl->mSet("ds_pousada", utf8_encode(nl2br($agencia[0]['ds_agencia'])));
            ...
            if($cUrl->mGetParam(2) == 3){
                ...
                utf8_encode(principalInicio("Agências / Guias > ". utf8_encode($agencia[0]['nm_cidade']) ." - Guia Aparados da Serra"));
                $cTpl->mSet("nm_cidade", utf8_encode($agencia[0]['nm_cidade']));
            }
            else{
                $agencia = $cAutomates->mRetornaDados("", "agencias.cd_cidade <> '3'", "agencias.cd_cidade ASC");
                utf8_encode(principalInicio("Agências / Guias > Outros - Guia Aparados da Serra"));
                $cTpl->mSet("nm_cidade", "Outros");
            }
            ...
            for ($i = 0; $i < count($agencia); $i++) {
                ...
                $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $agencia[$i]["ft_agencia"], 350, 1));
                ...
                $cTpl->mSet("nm_agencia", utf8_encode($agencia[$i]['nm_agencia']));
                $cTpl->mSet("end_agencia", utf8_encode($agencia[$i]['end_agencia']) . " - " . utf8_encode($agencia[$i]['nm_cidade']));
                ...
                $cTpl->mSet("ds_agencia", utf8_encode(nl2br($agencia[$i]['ds_agencia'])));
    ...
    case "onde-comer":
        ...
        if ($cUrl->mGetParam(2) == "enviar") {
            ...
            $cTpl = new cTemplate("tpl.onde-comer-enviar.htm");

            $cAutomates = new cAutomates("onde_comer");

            $ondeComer = $cAutomates->mRetornaDados($cUrl->mGetParam(3));

            principalInicio("Onde Comer > ". utf8_encode($ondeComer[0]['nm_cidade']) ." - Guia Aparados da Serra");

            $cTpl->mSet("nm_cidade", utf8_encode($ondeComer[0]['nm_cidade']));

            $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $ondeComer[0]["ft_comer"], 350, 1));
            $cTpl->mSet("link", $cGd->mGeraGd($caminhoFotos, $ondeComer[0]["ft_comer"], 500, 3));
            $cTpl->mSet("cd_comer", $ondeComer[0]['cd_comer']);
            $cTpl->mSet("nm_pousada", utf8_encode($ondeComer[0]['nm_comer']));
            $cTpl->mSet("link_comer", "/onde-comer/enviar/" . $ondeComer[0]['cd_comer']);
            $cTpl->mSet("end_pousada", utf8_encode($ondeComer[0]['end_comer']) . " - " . utf8_encode($ondeComer[0]['nm_cidade']));
            $cTpl->mSet("tel_pousada", $ondeComer[0]['tel_comer']);
            $cTpl->mSet("cel_pousada", $ondeComer[0]['cel_comer']);
            ...
            $cTpl->mSet("st_pousada", $link);
            $cTpl->mSet("ds_pousada", utf8_encode(nl2br($ondeComer[0]['ds_comer'])));
            ...
            for ($i = 0; $i < count($ondeComer); $i++) {
                ...
                $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $ondeComer[$i]["ft_comer"], 350, 1));
                $cTpl->mSet("link", $cGd->mGeraGd($caminhoFotos, $ondeComer[$i]["ft_comer"], 500, 3));
                $cTpl->mSet("cd_comer", $ondeComer[$i]['cd_comer']);
                $cTpl->mSet("nm_comer", utf8_encode($ondeComer[$i]['nm_comer']));
                $cTpl->mSet("link_comer", "/onde-comer/enviar/" . $ondeComer[$i]['cd_comer']);
                $cTpl->mSet("end_comer", utf8_encode($ondeComer[$i]['end_comer']) . " - " . utf8_encode($ondeComer[$i]['nm_cidade']));
                ...
                $cTpl->mSet("ds_comer", utf8_encode(nl2br($ondeComer[$i]['ds_comer'])));
    ...
    case "onde-ficar":
        ...
        if($cUrl->mGetParam(2) == "enviar"){
            ...
            principalInicio("Onde Ficar > ". utf8_encode($onde[0]['nm_cidade']) ." - Guia Aparados da Serra");

            $cTpl->mSet("nm_cidade", utf8_encode($onde[0]['nm_cidade']));
        
            $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $onde[0]["ft_ficar"], 350, 1));
            $cTpl->mSet("link", $cGd->mGeraGd($caminhoFotos, $onde[0]["ft_ficar"], 500, 3));

            $cTpl->mSet("nm_pousada", utf8_encode($onde[0]['nm_ficar']));
            $cTpl->mSet("end_pousada", utf8_encode($onde[0]['end_ficar']) . " - " . utf8_encode($onde[0]['nm_cidade']));
            $cTpl->mSet("tel_pousada", $onde[0]['tel_ficar']);
            $cTpl->mSet("cel_pousada", $onde[0]['cel_ficar']);
            ...
            $cTpl->mSet("st_pousada", utf8_encode($link));
            $cTpl->mSet("ds_pousada", utf8_encode(nl2br($onde[0]['ds_ficar'])));
            ...
            if ($onde[0]['em_ficar'] != "" && isset($onde[0]['em_ficar'])) {
                ...
                // require_once('zpainel/class/class.phpmailer.php');
                // $PHPMailer = new PHPMailer();
                ...
                // $PHPMailer->Host = "smtp.guiaaparadosdaserra.com.br";
                ...
                $assunto = "Sistema Automático Notificação";

                if (!mail($email,$assunto,$body,$headers, "-r"."sistema@guiaaparadosdaserra.com.br")){
            ...
            if($cUrl->mGetParam(2) == 3){
                ...
                principalInicio("Onde Ficar > ". utf8_encode($ondeFicar[0]['nm_cidade'] ." - Guia Aparados da Serra"));

                $cTpl->mSet("nm_cidade", utf8_encode($ondeFicar[0]['nm_cidade']));
            }
            else{
            ...
            for ($i = 0; $i < count($ondeFicar); $i++) {
                ...
                $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $ondeFicar[$i]["ft_ficar"], 350, 1));
                $cTpl->mSet("link", $cGd->mGeraGd($caminhoFotos, $ondeFicar[$i]["ft_ficar"], 500, 3));
                $cTpl->mSet("cd_ficar", utf8_encode($ondeFicar[$i]['cd_ficar']));
                $cTpl->mSet("link_ficar", "/onde-ficar/enviar/" . utf8_encode($ondeFicar[$i]['cd_ficar']));
                $cTpl->mSet("nm_ficar", utf8_encode($ondeFicar[$i]['nm_ficar']));
                $cTpl->mSet("end_ficar", utf8_encode($ondeFicar[$i]['end_ficar'] . " - " . $ondeFicar[$i]['nm_cidade']));
                $cTpl->mSet("tel_ficar", utf8_encode($ondeFicar[$i]['tel_ficar']));
                ...
                $cTpl->mSet("ds_ficar", utf8_encode(nl2br($ondeFicar[$i]['ds_ficar'])));
    ...
    case "parques-nacionais":
        ...
        if ($cUrl->mGetParam(2)) {
            ...
            $cTpl->mSet("titulo", utf8_encode($parques[0]['nm_parques']));
            $cTpl->mSet("descricao", utf8_encode(nl2br($parques[0]['ds_parques'])));
            $cTpl->mShow("inicio");
            ...
            $cTpl = new cTemplate("tpl.parques-nacionais.htm");
            $cTpl->mShow("inicio");
            ...
            for ($i = 0; $i < count($parques); $i++) {
                ...
                $cTpl->mSet("titulo", utf8_encode($parques[$i]['nm_parques']));
                $cTpl->mSet("descricao", utf8_encode($cAutomates->mBreveDescricao($parques[$i]['ds_parques'], 200)));
    ...
    case "aparados-da-serra-turismo":
        ...
        if ($cUrl->mGetParam(2)) {
            ...
            $cTpl->mSet("descricao", utf8_encode(nl2br($parques[0]['ds_turismo'])));
            $cTpl->mShow("inicio");
            ...
            $cTpl = new cTemplate("tpl.aparados-da-serra-turismo.htm");
            $cTpl->mShow("inicio");
            ...
            for ($i = 0; $i < count($parques); $i++) {
                ...
                $cTpl->mSet("titulo", utf8_encode($parques[$i]['nm_turismo']));
                $cTpl->mSet("descricao", utf8_encode($cAutomates->mBreveDescricao($parques[$i]['ds_turismo'], 200)));
    ...
    case "sobre-os-aparados-da-serra":
        ...
        $cTpl->mSet("ds_sobre_aparados", utf8_encode(nl2br($sobre_aparados[0]['ds_sobre_aparados'])));
        $cTpl->mShow("inicio");
    ...
    case "as-cidades":
        ...
        if ($cUrl->mGetParam(2)) {
            ...
            $cTpl->mSet("titulo", utf8_encode($cidades[0]['nm_cidades']));
            $cTpl->mSet("descricao", utf8_encode(nl2br($cidades[0]['ds_cidades'])));
            ...
            $cTpl = new cTemplate("tpl.as-cidades.htm");
            $cTpl->mShow("inicio");
            ...
            for ($i = 0; $i < count($cidades); $i++) {
                ...
                $cTpl->mSet("titulo", utf8_encode($cidades[$i]['nm_cidades']));
                $cTpl->mSet("descricao", utf8_encode($cAutomates->mBreveDescricao($cidades[$i]['ds_cidades'], 200)));
    ...
    case "produtos":
        ...
        if ($cUrl->mGetParam(2)) {
            ...
            $cTpl = new cTemplate("tpl.produtoss.htm");
            ...
            $cTpl->mSet("foto", "/../../".$cGd->mGeraGd($caminhoFotos, $produto[0]["ft_produto"], 350, 1));
            ...
            for ($i = 0; $i < count($produto); $i++) {
                ...
                $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $produto[$i]["ft_produto"], 350, 1));
    ...
    //     principalInicio("Telefones �teis - Guia Aparados da Serra");
    ...
    case "nivel-de-atividades":
        //include "principalInicio.php";
        principalInicio("N�vel de Atividades - Guia Aparados da Serra");
    ...
    case "trilha-dos-canyons":
        ...
        $cTpl->mSet("titulo", "Trilha dos Canyons e Cachoeiras 4x4");
		$cTpl->mSet("descricao", nl2br($trilha[0]['ds_excursao']));
        $cTpl->mShow("inicio");
    ...
    case "expedicao-aparados-4x4":
        ...
        $cTpl = new cTemplate("tpl.expedicao-4x4.htm");

        $cAutomates = new cAutomates("expedicao");
        $expedicao = $cAutomates->mRetornaDados();

        $cTpl->mSet("ds_expedicao", utf8_encode(nl2br($expedicao[0]['ds_expedicao'])));
        $cTpl->mShow("inicio");
    ...
    case "o-que-e-aparados-da-serra":
        ...
        $cTpl->mSet("ds_aparados", utf8_encode(nl2br($aparados[0]['ds_aparados'])));
        $cTpl->mShow("inicio");
    ...
    case "excursao-para-grupos-colegios-universidades":
        ...
        $cTpl = new cTemplate("tpl.excursao-para-grupo.htm");

        $cAutomates = new cAutomates("excursao");
        $excursao = $cAutomates->mRetornaDados();

        $cTpl->mSet("ds_excursao", utf8_encode(nl2br($excursao[0]['ds_excursao'])));
        $cTpl->mShow("inicio");
    ...
    case "as-cavalgadas-nos-aparados":
        ...
        $cTpl->mSet("ds_cavalgada", utf8_encode(nl2br($cavalgadas[0]['ds_cavalgada'])));
        $cTpl->mShow("inicio");
    ...
    case "os-trekkings-nos-aparados":
        ...
        $cTpl->mSet("ds_trekkings", utf8_encode(nl2br($trekkings[0]['ds_trekkings'])));
        $cTpl->mShow("inicio");
    ...
    case "mountain-bike-nos-aparados":
        ...
        $cTpl->mSet("ds_mountainbike", utf8_encode(nl2br($trekkings[0]['ds_mountainbike'])));
        $cTpl->mShow("inicio");
    ...
    case "atrativos":
        ...
        $cTpl->mSet("ds_atrativo", utf8_encode(nl2br($atrativos[0]['ds_atrativo'])));
        $cTpl->mShow("inicio");
    ...
    case "quem-somos":
        ...
        $cTpl->mSet("ds_quem_somos", utf8_encode(nl2br($quemSomos[0]['ds_quem_somos'])));
        $cTpl->mShow("inicio");
    ...
    default:
        ...
        for ($i=0; $i <count($destaques) ; $i++) { 
            $cTpl->mSet("img_destaque", "/fotos/foto.php?src=" . $destaques[$i]["arq_destaque"] ."&w=758&h=280&m=u");
            $cTpl->mSet("link", $destaques[$i]["link"]);

            $cTpl->mShow("home_slider");
        }
        $cTpl->mShow("end_home_slider");
        ...
        for ($i = 0; $i < count($noticia); $i++) {
            ...
            $cTpl->mSet("titulo", utf8_encode($noticia[$i]["tt_noticia"]));
        ...
        for ($i = 0; $i < count($informe); $i++) {
            $number = $i;
            $cTpl->mSet("count", $number+1);
            $cTpl->mSet("link", "informe-se/" . $informe[$i]["cd_informe"] . "/" . $cUrl->mTrataString($informe[$i]["tt_informe"]));
            $cTpl->mSet("titulo", utf8_encode(nl2br($informe[$i]["tt_informe"])));
            $cTpl->mSet("desc", utf8_encode($cAutomates->mBreveDescricao($informe[$i]["ds_informe"], 200)));
        ...
        for ($j = 0; $j < count($parceiro); $j++) {
            ...
            $cTpl->mSet("foto", $cGd->mGeraGd($caminhoFotos, $parceiro[$j]["ft_parceiro"], 288, 2));
            //$cTpl->mSet("foto", "/fotos/foto.php?src=" . $parceiro[$i]["ft_parceiro"] ."&w=288&h=280&m=u");
            $cTpl->mSet("link", "#1");
            if ($parceiro[$j]["st_parceiro"] != "") {
                $cTpl->mSet("link", $parceiro[$j]["st_parceiro"]);
            }

            $cTpl->mShow("while_parceiros");
        }

        $cTpl->mShow("fim_parceiros");
