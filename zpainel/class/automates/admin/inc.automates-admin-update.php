...
if ($_GET['dados']) {
	...
	foreach($dados as $key => $value) {
		...
		if ($basico == 0) $dados[$key] = "'".addslashes($value)."'";
	}
			...
			if ($campos[$i]["file"]) {
				...
				if (file_exists($this->v_caminho_imagem.$valorCampo)) {
					if (($extensaoArquivo == "jpg") || ($extensaoArquivo == "jpeg") || ($extensaoArquivo == "gif") || ($extensaoArquivo == "png")) {
						$cTpl->mSet("urlArquivoAmpliar","javascript:window.open('".$this->v_caminho_imagem.$valorCampo."');window.location='#';void(0);");
						...
						$cTpl->mSet("urlArquivoAmpliar",$this->v_caminho_imagem."download.php?arquivo=".$valorCampo);
					...
					$cTpl->mSet("urlArquivoAmpliar","javascript:window.open('".$this->v_caminho_imagem."max_".$valorCampo."');window.location='#';void(0);");
