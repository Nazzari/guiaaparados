 ...
 */
 class cTools {
	...
	/**
	 * Pega cotação de dolar e euro atraves do link http://www.debit.com.br/resumogratuito.php?info=novo_dolar
	 ...
	 * @return string » Valor da cotação solicitada
	 */
	function mMoedaCotacao($tipo=1) {
		//abre o arquivo
		if (!$fp = fopen("http://www.debit.com.br/resumogratuito.php?info=novo_dolar" ,"r" )) { print("Erro ao abrir a página de cotação"); exit();	}
	...
	/**
	 * Função dependente da mMoedaCotacao();
	 */
	...
	function mVerificaCamposBranco($p_array) {
		foreach($p_array as $s_conteudo) {
			if (trim($s_conteudo) == "") $this->mErro("Erro! Há campos obrigatórios em branco no formulário.");
		}
	}
	...
	/**
 	...
 	/**
 	 * Trata espaços em branco do array
 	 * @param array $p_texto » Array com os campos que deseja limpar os espaços
 	 * @return array » Array sem os espaçamentos
 	 */
 	 	...
 	 	if(!is_dir($p_dir)){
			...
			while ($s_valor = readdir($s_abrir)) {
				...
				for($i = 0; $i < count($p_tipoArquivo); $i++){
					 ...
					 // Não pegar os diretórios ateriores
					if (($s_valor != ".") && ($s_valor != "..")){

						 // Verifica a extensão do arquivo de acordo com o tipo especificado como parametro
	 ...
	 * for passado como parâmetro e retorna minúsculos
	 *
	...
	function mTrataCaracter($p_texto, $p_caracter="_"){
		return ereg_replace("[^a-zA-Z0-9 _.]", "", strtr($p_texto, "áàãâéêíóôõúüçÁÀÃÂÉÊÍÓÔÕÚÜÇ'´`. ", "aaaaeeiooouucAAAAEEIOOOUUC   _ $p_caracter"));
	}
	...
	/**
	 * Retorna o dia da semana em protuguês,
	 ...
	 * @param string $padrao » Se padrao for americano, data é "2013-12-01" $padrao=1 
	 * senão é padrão brasileiro (01-12-2013)
	 */
	...
	/**
	 * Retorna  o mês por extenso em portugues, em caso de erro retorna
	 * mensagem de erro
	 * @param string $p_mes_por_extenso » date("F") ou date("m") ou date("M"), ou em ingles
	 * @return string » Mês por extenso em português
	 */
	function mMesExtensoBr($p_mes_por_extenso){
		...
		elseif(($p_mes_por_extenso == "March") || ($p_mes_por_extenso == "03") || ($p_mes_por_extenso == "Mar")){ $p_mes = "Março"; }
		...
		if(empty($p_mes)) exit("Erro - Não existe esse Mês!");
		else return $p_mes;
	}

	/**
	 * Exibe menssagem de erro, e usa um javascript para
	 * retornar a página anterior através de um link voltar e interrompe
	 * o fluxo do script
	 * @param string $p_msg » Mensagem que quer que apareca para o usuário
	 */
	 ...
	 * @param string $p_destino » Link de destino da página (para onde queres ir)
	 */
	 ...
	 * @param array $p_tags » Array com as tags - 0 caso queira o padrão
	 * @return string » Palavra sem as tags
	 */
	function mRemoveTags ($p_texto, $p_tags=0) {

		// Caso tags = 0, ele seta os arrays de tags padrão
	...
	function mListaEstadoUF($p_tipo=1) {
		...
		if($p_tipo == 1) {
			...
			$uf_estado = array("Acre", "Alagoas", "Amapá", "Amazonas", "Bahia",
						   "Ceará", "Distrito Federal", "Goiás", "Espírito Santo",
						   ...
						   "Rio Grande do Sul", "Rondônia", "Rorâima", "São Paulo",
