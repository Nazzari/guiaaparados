...
switch($cUrl->mGetParam(1)) {
	...
	**************************/
	case "fazendas-roteiros":
		...
		if ($cUrl->mGetParam(2)) {
			...
			$cTpl = new cTemplate("tpl.pousadasInt.htm");
			$cTpl->mShow("inicio");
			...
			$cTpl = new cTemplate("tpl.pousadas.htm");
			$cTpl->mShow("inicio");
			...
			for ($i=0; $i < count($pousadas); $i++) { 
				...
				$cTpl->mSet("descricao",$cAutomates->mBreveDescricao($pousadas[$i]['ds_pousadas'],400));
				$cTpl->mShow("dados");
			}
	...
	**************************/
	case "perguntas-e-respostas":
		...
		$cTpl = new cTemplate("tpl.perguntas.htm");
